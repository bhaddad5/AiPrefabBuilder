...
public class ToolsImplementation : IToolsImplementation
{
	...
	public (string Info, Dictionary<string, BinaryData> Renders) AnalyzeInstructions(string instructions)
	{
		...
		int numFinalBuilds = Directory.GetFiles($"{folder}/Tmp").Where(f => f.EndsWith(".prefab")).ToList().Count;

		string prefabPath = $"{folder}/Tmp/Final Build Attempt {numFinalBuilds}.prefab";
		...
		string boundsStr = $"Bounds(min:{res.Bounds.min},max{res.Bounds.max})";
